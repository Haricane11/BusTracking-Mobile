import React, { useEffect, useState, useMemo, useCallback } from 'react' // Import useMemo and useCallback
import { View, Text, FlatList, StyleSheet, TextInput, TouchableOpacity } from 'react-native'
import Svg, { Path } from 'react-native-svg';
import { useMyContext } from '../useContext/UseContext'

export default function Nearby() {
    const { activeNav, nearby, setSpecificBusStop } = useMyContext();
    const [displayedBusStops, setDisplayedBusStops] = useState([]);
    const [searchQuery, setSearchQuery] = useState("");
    const BATCH_SIZE = 500;

    // 🌟 1. Deduplicate the nearby list first 🌟
    const uniqueNearbyStops = useMemo(() => {
        if (!nearby || nearby.length === 0) return [];
        
        const seenIds = new Set();
        // Assuming your bus stop objects have a unique property called 'id'
        return nearby.filter(stop => {
            // Ensure the stop has an ID and it hasn't been seen yet
            if (stop.id && !seenIds.has(stop.id)) {
                seenIds.add(stop.id);
                return true;
            }
            return false;
    ...
    // Note: If your unique key is NOT 'id', replace 'stop.id' with the correct property.
    ...
    // 2. Filter based on the UNIQUE list
    const currentlyFilteredList = useMemo(
        // Use the uniqueNearbyStops here instead of the raw 'nearby' list
        () => filterBusStops(uniqueNearbyStops, searchQuery),
        [uniqueNearbyStops, searchQuery, filterBusStops]
    );

    // 3. Effect to handle pagination on filter/data change
    useEffect(() => {
        // This is simplified now that currentlyFilteredList is memoized and always unique
        setDisplayedBusStops(currentlyFilteredList.slice(0, BATCH_SIZE));
    }, [searchQuery, currentlyFilteredList]); 
    // Removed 'nearby' and 'filterBusStops' from deps since we depend on 'currentlyFilteredList'
            ...
            <View style={styles.busStopsContentWrapper}>
                ...
                {currentlyFilteredList.length > 0 ? (
                    ...
                    <FlatList
                        ...
                        // 🌟 4. Switched keyExtractor to use 'item.id' 🌟
                        // This is necessary because the data is now deduplicated by 'id'
